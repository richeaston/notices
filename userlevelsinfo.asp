...
' ASPMaker configuration for Table UserLevels
Dim UserLevels
...
Class cUserLevels
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "UserLevelName", UserLevelName)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "UserLevels_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [UserLevels]"
	End Property
	...
	Private Property Get SqlKeyFilter()
		SqlKeyFilter = "[UserLevelID] = @UserLevelID@"
	End Property
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "userlevelslist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "userlevelslist.asp"
	End Function

	' View url
	Public Function ViewUrl()
		ViewUrl = KeyUrl("userlevelsview.asp", UrlParm(""))
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("userlevelsedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("userlevelsadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("userlevelsdelete.asp", UrlParm(""))
	End Function
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("UserLevelID") ' UserLevelID
	...
	Public Sub LoadListRowValues(RsRow)
		...
		UserLevelName.DbValue = RsRow("UserLevelName")
	End Sub
	...
	Sub RenderListRow()
		...
		' UserLevelName
		UserLevelName.LinkCustomAttributes = ""
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					...
					Call XmlDoc.AddField("UserLevelName", UserLevelName.ExportValue(Export, ExportOriginalValue))
				Else
					Call XmlDoc.AddField("UserLevelID", UserLevelID.ExportValue(Export, ExportOriginalValue))
					Call XmlDoc.AddField("UserLevelName", UserLevelName.ExportValue(Export, ExportOriginalValue))
				End If
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				...
				Call Doc.ExportCaption(UserLevelName)
			Else
				Call Doc.ExportCaption(UserLevelID)
				Call Doc.ExportCaption(UserLevelName)
			End If
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					...
					Call Doc.ExportField(UserLevelName)
				Else
					Call Doc.ExportField(UserLevelID)
					Call Doc.ExportField(UserLevelName)
				End If
...
'			End If
	...
	Public Property Get UserLevelID()
		If Not IsObject(m_UserLevelID) Then
			Set m_UserLevelID = NewFldObj("UserLevels", "UserLevels", "x_UserLevelID", "UserLevelID", "[UserLevelID]", 3, 7, "", False, False, "FORMATTED TEXT")
	...
	' Field UserLevelName
	Private m_UserLevelName

	Public Property Get UserLevelName()
		If Not IsObject(m_UserLevelName) Then
			Set m_UserLevelName = NewFldObj("UserLevels", "UserLevels", "x_UserLevelName", "UserLevelName", "[UserLevelName]", 202, 7, "", False, False, "FORMATTED TEXT")
		End If
		Set UserLevelName = m_UserLevelName
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		...
		If IsObject(m_UserLevelName) Then Set m_UserLevelName = Nothing
